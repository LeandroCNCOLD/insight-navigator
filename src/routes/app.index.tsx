...
import { FileText, DollarSign, Users, Building2, TrendingUp, Wrench } from "lucide-react";
...
function Dashboard() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [docs, props, clients, comps, equips, recent] = await Promise.all([
        ...
        supabase.from("proposals").select("id,valor_total,data_proposta,status_proposta,competitor_id,client_id"),
        ...
        supabase.from("documents").select("id,file_name,status,created_at,competitor:competitors(nome)").order("created_at", { ascending: false }).limit(8),
      ]);
      return { docs, props, clients, comps, equips, recent };
    },
  });

  const totalValor = (data?.props.data || []).reduce((s, p) => s + (Number(p.valor_total) || 0), 0);
  const ticket = data?.props.data?.length ? totalValor / data.props.data.length : 0;

  // Monthly evolution
  const monthly: Record<string, number> = {};
  (data?.props.data || []).forEach((p) => {
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader title="Dashboard" description="Visão consolidada da inteligência competitiva." />

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <KpiCard label="Documentos" value={formatNumber(data?.docs.count || 0)} icon={FileText} />
        <KpiCard label="Propostas" value={formatNumber(data?.props.data?.length || 0)} icon={FileText} />
          ...
          {(data?.recent.data || []).map((d: any) => (
            <Link key={d.id} to="/app/documents/$id" params={{ id: d.id }} className="flex items-center gap-3 px-5 py-3 hover:bg-muted/30">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground">{d.competitor?.nome || "—"} · {new Date(d.created_at).toLocaleString("pt-BR")}</div>
              </div>
