...
export const Route = createFileRoute("/app/equipments")({
  ...
  head: () => ({
    meta: [{ title: "Equipment Intelligence — DocIntel" }],
  }),
});
...
function EquipmentsIntelligencePage() {
  ...
  if (isLoading) {
    return <div className="p-6 text-sm text-muted-foreground">Carregando inteligência de equipamentos...</div>;
  }
    ...
    return (
      <div className="p-6">
        <PageHeader
          ...
          description="Padrões técnicos, concentração regional e leitura analítica da base de equipamentos."
        />
        <EmptyState
          icon={Wrench}
          title="Sem equipamentos estruturados"
          description="Envie documentos e gere propostas com equipamentos para ativar este módulo."
          action={
            <Link to="/app/upload">
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        title="Equipment Intelligence"
        description="Motor analítico para padrões técnicos, distribuição, recorrência e benchmarking de equipamentos."
      />
      ...
      <Card className="p-4">
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Buscar por modelo, proposta, cliente, estado, concorrente, gás, compressor..."
            className="pl-9"
          />
        </div>
      </Card>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList className="flex flex-wrap">
          ...
          <TabsTrigger value="patterns">Padrões Técnicos</TabsTrigger>
          <TabsTrigger value="regions">Regiões e Concorrentes</TabsTrigger>
          <TabsTrigger value="proposals">Por Proposta</TabsTrigger>
          <TabsTrigger value="catalog">Catálogo Analítico</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="patterns">
          <div className="grid gap-4 lg:grid-cols-2">
            <ChartCard title="Combos técnicos mais recorrentes" data={data.topTechnicalCombos} dataKey="value" nameKey="label" />
            <Card className="p-5">
              <div className="mb-4 text-lg font-semibold">Insights automáticos</div>
        ...
        <TabsContent value="regions">
          <div className="grid gap-4 lg:grid-cols-2">
            <ChartCard title="Concentração por estado" data={data.byState} dataKey="value" nameKey="label" />
            <ChartCard title="Concentração por concorrente" data={data.byCompetitor} dataKey="value" nameKey="label" />
          </div>
        </TabsContent>

        <TabsContent value="proposals">
          <Card className="p-5">
            <div className="mb-4 text-lg font-semibold">Resumo técnico por proposta</div>
        ...
        <TabsContent value="catalog">
          <Card className="p-5">
            <div className="mb-4 flex items-center justify-between">
              <div className="text-lg font-semibold">Catálogo analítico de equipamentos</div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/50">
                  <tr className="text-left">
                    ...
                    <th className="px-4 py-3 font-medium">Gás</th>
                ...
                <tbody>
                  {filteredRows.map((row) => (
                    <tr key={row.id}>
                      <td className="border-t px-4 py-3">{row.modelo || row.tipo || "—"}</td>
                      <td className="border-t px-4 py-3">{row.quantidade || 1}</td>
                      <td className="border-t px-4 py-3">{row.potencia_hp || "—"}</td>
                      <td className="border-t px-4 py-3">{row.capacidade_kcal || "—"}</td>
                      <td className="border-t px-4 py-3">{row.gas_refrigerante || "—"}</td>
                      <td className="border-t px-4 py-3">{row.compressor || "—"}</td>
                      <td className="border-t px-4 py-3">{row.proposal?.client?.nome || "—"}</td>
                      <td className="border-t px-4 py-3">{row.proposal?.client?.estado || "—"}</td>
                      <td className="border-t px-4 py-3">{row.proposal?.competitor?.nome || "—"}</td>
                      <td className="border-t px-4 py-3">{row.proposal?.numero || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
